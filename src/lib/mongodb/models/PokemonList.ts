...
import { PokemonList, CardInList, ListType } from '@/types/pokemon';

const COLLECTION_NAME = 'pokemon_lists';

export async function getListsCollection(): Promise<Collection<PokemonList>> {
  const db = await getDatabase();
  return db.collection<PokemonList>(COLLECTION_NAME);
}
...
export async function getOrCreateList(type: ListType, userId: string): Promise<PokemonList> {
  const collection = await getListsCollection();

  let list = await collection.findOne({ type, userId });

  if (!list) {
    const newList: Omit<PokemonList, '_id'> = {
      type,
      userId,
      cards: [],
      createdAt: new Date(),
...
export async function addCardToList(
  ...
  // Use cardId as unique identifier (Pokemon IDs are strings like "swsh4-25")
