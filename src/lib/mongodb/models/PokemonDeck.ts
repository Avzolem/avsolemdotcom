...
import { PokemonDeck, CardInDeck, DeckZone, DECK_LIMITS, isBasicEnergy } from '@/types/pokemon';

const COLLECTION_NAME = 'pokemon_decks';
...
function toDeck(doc: Document): PokemonDeck {
  return { ...doc, _id: doc._id?.toString() } as unknown as PokemonDeck;
}
...
export async function getUserDecks(userId: string): Promise<PokemonDeck[]> {
  const col = await getCollection();
  const docs = await col.find({ userId }).sort({ updatedAt: -1 }).toArray();
  return docs.map(toDeck);
}

export async function getDeckById(deckId: string, userId: string): Promise<PokemonDeck | null> {
  const col = await getCollection();
...
export async function createDeck(
  ...
  name: string,
  description?: string
): Promise<PokemonDeck | { error: string }> {
  const col = await getCollection();
  ...
  return { ...newDeck, _id: result.insertedId.toString() } as PokemonDeck;
}
 ...
 * Pokemon rules: 60 cards exactly, max 4 copies (Basic Energy unlimited)
 */
function validateAddCard(
  ...
  if (totalCards + quantity > DECK_LIMITS.DECK_SIZE) {
    return 'DECK_FULL';
  }

  // Basic Energy has no copy limit
  if (isBasicEnergy(card.cardName, card.subtypes)) {
    return null;
  }

  // Check max copies (4 for non-Basic Energy)
  const totalCopies = deck.cards
    .filter((c) => c.cardName === card.cardName) // Compare by name, not ID (different prints of same card)
...
export async function addCardToDeck(
  ...
  const validationError = validateAddCard(deck, card, quantity);
