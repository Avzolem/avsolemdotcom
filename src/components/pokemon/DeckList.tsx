...
import { PokemonDeck, DECK_LIMITS } from '@/types/pokemon';
...
export default function DeckList() {
  ...
  const fetchDecks = useCallback(async () => {
    try {
      const res = await fetch('/api/pokemon/decks');
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      setNewDeckName('');
      setNewDeckDesc('');
      router.push(`/pokemon/decks/${data.deck._id}`);
    } catch {
      showToast(t('deck.error.create'), 'error');
    } finally {
      setCreating(false);
  ...
  const handleDelete = async (deckId: string, deckName: string) => {
    ...
    try {
      const res = await fetch(`/api/pokemon/decks/${deckId}`, { method: 'DELETE' });
      if (res.ok) {
  ...
  const getDeckCardCount = (deck: PokemonDeck) => {
    const pokemon = deck.cards.filter((c) => c.zone === 'pokemon').reduce((s, c) => s + c.quantity, 0);
    const trainer = deck.cards.filter((c) => c.zone === 'trainer').reduce((s, c) => s + c.quantity, 0);
    const energy = deck.cards.filter((c) => c.zone === 'energy').reduce((s, c) => s + c.quantity, 0);
          ...
          {decks.map((deck) => {
            ...
            return (
              <div
                ...
                onClick={() => router.push(`/pokemon/decks/${deck._id}`)}
              >
                ...
                <div className={styles.deckInfo}>
                  ...
                  <div className={styles.deckCounts}>
                    <span className={styles.countBadge}>Pokemon: {counts.pokemon}</span>
                    <span className={styles.countBadge}>Trainer: {counts.trainer}</span>
                    <span className={styles.countBadge}>Energy: {counts.energy}</span>
                  </div>
                  <div className={styles.deckTotal}>
                    <span className={`${styles.totalBadge} ${counts.total === DECK_LIMITS.DECK_SIZE ? styles.totalValid : styles.totalInvalid}`}>
                      Total: {counts.total}/{DECK_LIMITS.DECK_SIZE}
                    </span>
                  </div>
