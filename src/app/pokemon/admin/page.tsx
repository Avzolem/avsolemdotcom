...
export default function AdminPage() {
  const { user, isAuthenticated, isLoading } = usePokemonAuth();
  const { showToast } = useToast();
  const { language, t } = usePokemonLanguage();
  ...
  const fetchProducts = useCallback(async () => {
    try {
      const res = await fetch('/api/pokemon/admin/products');
      if (res.ok) {
  ...
  const fetchNews = useCallback(async () => {
    try {
      const res = await fetch('/api/pokemon/admin/news');
      if (res.ok) {
        const data = await res.json();
        setNews(data.news || []);
      }
  ...
  const handleSendEmail = async () => {
    ...
    try {
      const res = await fetch('/api/pokemon/admin/send-email', {
        method: 'POST',
  ...
  const handleDeleteProduct = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/pokemon/admin/products?id=${id}`, { method: 'DELETE' });
      if (res.ok) {
  ...
  const handleDeleteNews = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/pokemon/admin/news?id=${id}`, { method: 'DELETE' });
      if (res.ok) {
