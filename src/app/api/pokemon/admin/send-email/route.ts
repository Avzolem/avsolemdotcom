...
function markdownToHtml(markdown: string, userName?: string): string {
  let html = markdown
    ...
    // Headers
    .replace(/^### (.*$)/gim, '<h3 style="color: #EE1515; margin: 20px 0 10px 0;">$1</h3>')
    .replace(/^## (.*$)/gim, '<h2 style="color: #EE1515; margin: 25px 0 12px 0;">$1</h2>')
    .replace(/^# (.*$)/gim, '<h1 style="color: #EE1515; margin: 30px 0 15px 0;">$1</h1>')
    // Bold
        ...
        <table role="presentation" width="600" cellspacing="0" cellpadding="0" style="max-width: 600px;">
          ...
          <tr>
            <td align="center" style="padding-bottom: 30px;">
              <h1 style="color: #FFCB05; font-size: 28px; margin: 0; text-shadow: 2px 2px 0px #3B5CA8;">Pokemon TCG Manager</h1>
            </td>
          </tr>
          <!-- Content -->
          <tr>
            <td style="background: linear-gradient(135deg, #16213e 0%, #0f3460 100%); border: 2px solid #3B5CA8; border-radius: 16px; padding: 40px;">
          ...
          <tr>
            <td align="center" style="padding-top: 30px;">
              <p style="color: rgba(255, 255, 255, 0.5); font-size: 12px; margin: 0;">
                Pokemon TCG Manager by <a href="https://avsolem.com" style="color: #FFCB05;">avsolem.com</a>
              </p>
...
export async function POST(request: NextRequest) {
  try {
    ...
    const fromAddress = process.env.RESEND_FROM || 'Pokemon TCG Manager <onboarding@resend.dev>';
