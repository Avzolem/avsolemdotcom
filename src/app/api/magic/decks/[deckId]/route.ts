...
import {
  ...
  removeCardFromDeck,
  deleteDeck,
} from '@/lib/mongodb/models/MagicDeck';
import { DeckZone } from '@/types/magic';
...
export async function PUT(
  ...
  try {
    ...
    // Default: update metadata (name, description, format)
    const { name, description, format } = body;
    const success = await updateDeckMeta(deckId, userId, { name, description, format });
    if (!success) {
      return NextResponse.json({ message: 'Deck not found' }, { status: 404 });
    }
