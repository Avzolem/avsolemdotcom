...
function markdownToHtml(markdown: string, userName?: string): string {
  let html = markdown
    ...
    .replace(/\{nombre\}/gi, userName || 'Planeswalker')
    // Headers
    .replace(/^### (.*$)/gim, '<h3 style="color: #D4AF37; margin: 20px 0 10px 0;">$1</h3>')
    .replace(/^## (.*$)/gim, '<h2 style="color: #D4AF37; margin: 25px 0 12px 0;">$1</h2>')
    .replace(/^# (.*$)/gim, '<h1 style="color: #D4AF37; margin: 30px 0 15px 0;">$1</h1>')
    // Bold
    .replace(/\*\*(.*?)\*\*/gim, '<strong>$1</strong>')
    // Italic
    .replace(/\*(.*?)\*/gim, '<em>$1</em>')
    // Links
    .replace(/\[([^\]]+)\]\(([^)]+)\)/gim, '<a href="$2" style="color: #0E68AB; text-decoration: underline;">$1</a>')
    // Unordered lists
...
<body style="margin: 0; padding: 0; background-color: #2e0a0a; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;">
  <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background-color: #2e0a0a;">
    <tr>
        ...
        <table role="presentation" width="600" cellspacing="0" cellpadding="0" style="max-width: 600px;">
          ...
          <tr>
            <td align="center" style="padding-bottom: 30px;">
              <h1 style="color: #D4AF37; font-size: 28px; margin: 0; text-shadow: 2px 2px 0px #180505;">Magic: The Gathering Manager</h1>
            </td>
          </tr>
          <!-- Content -->
          <tr>
            <td style="background: linear-gradient(135deg, #2e0a0a 0%, #180505 100%); border: 2px solid rgba(212, 175, 55, 0.3); border-radius: 16px; padding: 40px;">
          ...
          <tr>
            <td align="center" style="padding-top: 30px;">
              <p style="color: rgba(255, 255, 255, 0.5); font-size: 12px; margin: 0;">
                Magic: The Gathering Manager by <a href="https://avsolem.com" style="color: #D4AF37;">avsolem.com</a>
              </p>
