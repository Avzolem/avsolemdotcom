...
import {
  ...
  updateProduct,
  deleteProduct,
} from '@/lib/mongodb/models/MagicProduct';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const product = await createProduct({
      ...
      inStock: isActive !== false && (stock || 1) > 0,
      link: '',
    });
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (updates.isActive !== undefined || updates.stock !== undefined) {
      updates.inStock = updates.isActive !== false && (updates.stock || 1) > 0;
