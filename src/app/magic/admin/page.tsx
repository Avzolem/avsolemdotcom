...
export default function AdminPage() {
  const { user, isAuthenticated, isLoading } = useMagicAuth();
  const { showToast } = useToast();
  const { language, t } = useMagicLanguage();
  ...
  const fetchAdminData = useCallback(async () => {
    try {
      const response = await fetch('/api/magic/admin');
      if (response.ok) {
  ...
  const fetchProducts = useCallback(async () => {
    try {
      const res = await fetch('/api/magic/admin/products');
      if (res.ok) {
  ...
  const fetchNews = useCallback(async () => {
    try {
      const res = await fetch('/api/magic/admin/news');
      if (res.ok) {
        const data = await res.json();
        setNews(data.news || []);
      }
  ...
  const handleSendEmail = async () => {
    ...
    try {
      const res = await fetch('/api/magic/admin/send-email', {
        method: 'POST',
  ...
  const handleDeleteProduct = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/magic/admin/products?id=${id}`, { method: 'DELETE' });
      if (res.ok) {
  ...
  const handleDeleteNews = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/magic/admin/news?id=${id}`, { method: 'DELETE' });
      if (res.ok) {
      ...
      {activeTab === 'stats' && stats && (
        <div className={styles.statsSection}>
          <div className={styles.statsGrid}>
            <div className={styles.statCard}><span className={styles.statValue}>{stats.totalUsers}</span><span className={styles.statLabel}>{t('admin.stats.totalUsers')}</span></div>
            <div className={styles.statCard}><span className={styles.statValue}>{stats.newsletterSubscribers}</span><span className={styles.statLabel}>{t('admin.stats.newsletter')}</span></div>
            <div className={styles.statCard}><span className={styles.statValue}>{stats.googleUsers}</span><span className={styles.statLabel}>{t('admin.stats.google')}</span></div>
            <div className={styles.statCard}><span className={styles.statValue}>{stats.credentialsUsers}</span><span className={styles.statLabel}>{t('admin.stats.emailPass')}</span></div>
            <div className={styles.statCard}><span className={styles.statValue}>{stats.usersThisMonth}</span><span className={styles.statLabel}>{t('admin.stats.thisMonth')}</span></div>
            <div className={styles.statCard}><span className={styles.statValue}>{stats.usersThisWeek}</span><span className={styles.statLabel}>{t('admin.stats.thisWeek')}</span></div>
          </div>
        </div>
      )}
